...
public class Spawn {
	...
	// Every 250 points, it goes to next level. More enemies spawn every level and
	// new types as well.
	public void tick() {
		scoreKeep++;
		...
		if (scoreKeep >= 250) {
			...
			// Switch is used where every case is a new level, with enemies spawning every
			// level, and new enemies spawning aswell.
			...
			case 3:
				handler.addObject(new RepeaterEnemy(r.nextInt(Game.WIDTH - 50), r.nextInt(Game.HEIGHT - 50),
						ID.RepeaterEnemy, handler));
				break;
